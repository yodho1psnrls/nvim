...
#include "../try2/garbage_policy.hpp"
...
public:
  ...
  bool contains(vert_key key) const {
    return 0 <= key && key < size<vert_key>();
  }

  bool contains(edge_key key) const {
    return 0 <= key && key < size<edge_key>();
  }
  ...
  template <typename KeyType>
  index_iterator<KeyType> begin() const;
  
  template <typename KeyType>
  index_iterator<KeyType> end() const;
  ...
  template <typename KeyType>
  Iterable<index_iterator<KeyType>> range() const;
...
, public IndexGarbagePolicy<AdjacencyList<V, E>, BaseAdjacencyList::vert_key>
, public IndexGarbagePolicy<AdjacencyList<V, E>, BaseAdjacencyList::edge_key> {
