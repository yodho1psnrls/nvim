...
//
// TODO: Make the Layers as an inheritance hierarchy, templating only the
// Iterator type, and getting rid of the type aliases and long error messages
//
...
template <typename RangeLeft, typename RangeRight, typename =
  std::enable_if_t<is_range_v<RangeLeft> && is_range_v<RangeRight>>>
inline bool operator==(const RangeLeft& lhs, const RangeRight& rhs) {
  if(lhs.size() != rhs.size())
    throw std::out_of_range("The sizes of the two ranges to compare, are not the same");
...
public:
  ...
  template <typename ViewType, typename = std::enable_if_t<
    std::is_constructible_v<BaseViewType, ViewType>>>
  InputViewLayer(const InputViewLayer<ViewType>& other)
  : BaseViewType(static_cast<const ViewType&>(other)) {}
  
  // BUG: Interferes with the single argument constructors
  // of the TensorIterator Adaptors
  // template <typename Range, typename = std::enable_if_t<
  //   is_range_v<std::remove_reference_t<Range>>>>
  // InputViewLayer(Range&& other)
  // : BaseViewType(other.begin(), other.end()) {}
...
// InputViewLayer(Range&&)
// -> InputViewLayer<BaseView<range_to_iterator_t<std::remove_reference_t<Range>>>>;
...
// template <typename Range>
// OutputViewLayer(Range&&)
// -> OutputViewLayer<InputViewLayer<BaseView<range_to_iterator_t<std::remove_reference_t<Range>>>>>;
...
//
// template <typename Cont, typename = std::enable_if_t<is_container_v<Cont>>>
// inline std::ostream& operator<<(std::ostream& stream, const Cont& cont) {
...
// inline auto movable(Cont&& cont) {
//  using C = std::remove_reference_t<Cont>;
//
//  static_assert(is_range_v<C>);
//  static_assert(!is_const_iterator_v<std::remove_cvref_t<decltype(std::declval<C>().begin())>>);
//
//  // NOTE: You would need to specialize std::move_iterator of TensorIterator !
//   return View(
//     std::make_move_iterator(std::forward<Cont>(cont).begin()),
//     std::make_move_iterator(std::forward<Cont>(cont).end())
//   );
// }

template <typename Cont>
inline auto reversed(Cont&& cont) {
  using C = std::remove_reference_t<Cont>;
  ...
  return View(
    std::make_reverse_iterator(std::forward<Cont>(cont).end()),
    std::make_reverse_iterator(std::forward<Cont>(cont).begin())
  );
}
