...
public:
  ...
  key_iterator insert(key_iterator key_it) {
    key_it = _pol.insert(key_it);
    auto row_ids = _pol.at(*key_it);
    _flat.insert(row_ids.begin(), row_ids.size());
    return key_it;
  }
  
  key_iterator insert(key_iterator key_it, size_type num_of_rows) {
    key_it = _pol.insert(key_it, num_of_rows);
    size_type n = std::distance(
      _pol.at(*key_it).begin(),
      _pol.at(*(key_it + num_of_rows)).begin()
    );
    auto row_ids = _pol.at(*key_it);
    _flat.insert(row_ids.begin(), n);
    return key_it;
  }

  key_iterator erase(key_iterator key_it) {
    auto row_ids = _pol.at(*key_it);
    _flat.erase(row_ids.begin(), row_ids.end());
    return _pol.erase(key_it);
  }
  ...
  key_iterator erase(key_iterator from, key_iterator to) {
    ...
    return _pol.erase(from, to);
  }


  flat_key_iterator insert_in(key_iterator key_it, flat_key_iterator flat_it) {
    flat_it = _flat.insert(flat_it);
    _pol.insert_in(key_it, flat_it);
    return flat_it;
  }
  
  flat_key_iterator insert_in(key_iterator key_it, flat_key_iterator flat_it, size_type num_of_vals) {
    flat_it = _flat.insert(flat_it, num_of_vals);
    _pol.insert_in(key_it, flat_it, num_of_vals);
    return flat_it;
  }

  flat_key_iterator erase_in(key_iterator key_it, flat_key_iterator flat_it) {
    _pol.erase_in(key_it, flat_it);
    return _flat.erase(flat_it);
  }

  flat_key_iterator erase_in(key_iterator key_it, flat_key_iterator flat_from, flat_key_iterator flat_to) {
    _pol.erase_in(key_it, flat_from, flat_to);
    return _flat.erase(flat_from, flat_to);
  }
