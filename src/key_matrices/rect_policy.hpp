...
// template <typename FlatKeyIter, bool DYNAMIC_ROW_SIZE = false>
class KeyRectMatPolicy {
public:

  using difference_type = std::iter_difference_t<FlatKeyIter>;
  using size_type = std::make_unsigned_t<difference_type>;

  using flat_type = std::iter_value_t<FlatKeyIter>;
  using flat_reference = std::iter_reference_t<FlatKeyIter>;
  using flat_iterator = FlatKeyIter;
  ...
  // static_assert(is_const_iterator_v<FlatKeyIter>);
  // static_assert(is_key_handle_v<flat_type>);
  static_assert(std::is_integral_v<flat_type>);
  // static_assert(std::is_same_v<index_handle_category,
  //   typename handle_traits<ValKey>::handle_category>);


  KeyRectMatPolicy(FlatKeyIter last, difference_type row_size)
  : _begin(last), _end(last), _n(row_size) {}
  
  KeyRectMatPolicy(FlatKeyIter first, FlatKeyIter last, difference_type row_size)
  : _begin(first), _end(last), _n(row_size) {}
  ...
  iterator begin() const {
    return iterator(_begin, _n);
  }

  iterator end() const {
    return iterator(_end, _n);
  }

  reference operator[](difference_type id) const {
    return reference(_begin + id * _n, _n);
  }

  reference at(difference_type id) const {
    flat_iterator it = _begin + id * _n;

    if (it < _begin || it >= _end)
      throw std::out_of_range("Out of range matrix index key");

    return reference(it, _n);
  }
  ...
  iterator insert(iterator key_it, difference_type num_of_rows = 1) {
    std::advance(_end, _n * num_of_rows);
    return key_it;
  }
  ...
  iterator erase(iterator key_it, difference_type num_of_rows = 1) {
    std::advance(_end, -_n * num_of_rows);
    return key_it;
  }
